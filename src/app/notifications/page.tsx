        ...
        <div className="min-h-screen py-12">
            <div className="max-w-2xl mx-auto px-4">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h1 className="text-3xl font-display font-semibold text-slate-900">Notifications</h1>
                        <p className="text-slate-600 mt-1">Stay updated on your activity</p>
                    </div>
                    {notifications.some(n => !n.is_read) && (
                        <button className="px-4 py-2 text-teal-700 font-semibold hover:bg-teal-50 rounded-xl transition text-sm">
                            Mark all as read
                        </button>
                    )}
                </div>
                    ...
                    <div className="surface overflow-hidden divide-y divide-slate-100">
                        {notifications.map((notification) => (
                            <Link
                                ...
                                className={`block p-5 hover:bg-slate-50 transition ${!notification.is_read ? 'bg-teal-50/60' : ''
                                    }`}
                            >
                                <div className="flex items-start gap-4">
                                    ...
                                    {!notification.is_read && (
                                        <div className="w-3 h-3 bg-teal-500 rounded-full flex-shrink-0 mt-1.5" />
                                    )}
                                </div>
                            </Link>
                        ))}
